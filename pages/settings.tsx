...
// Settings Page
const SettingsPage: React.FC<SettingsPageProps> = ({ swal }) => {
  const [products, setProducts] = useState<ProductType[]>([]);
  const [featuredProduct, setFeaturedProduct] = useState<string>('');
  const [shippingFee, setShippingFee] = useState<number>(0);
  const [shopName, setShopName] = useState<string>('');
  const [coupons, setCoupons] = useState<CouponType | {}>({});
  const [couponTypes, setCouponTypes] = useState<CouponState>({});
  const [couponApplyTo, setCouponApplyTo] = useState<CouponState>({});
  const [couponProduct, setCouponProduct] = useState<CouponState>({});
  const [couponCategory, setCouponCategory] = useState<CouponState>({});
  ...
  async function saveSettings() {
    // Format coupon data
    const processedCoupons: CouponState = Object.keys(coupons).reduce(
      (acc, couponName) => {
        const couponType: string = couponTypes[couponName];
        acc[couponName] = {
    ...
    for (let couponID in processedCoupons) {
      let coupon: CouponType = coupons[couponID];
      if (!coupon.code) {
        await swal.fire({
  ...
  function addCoupon() {
    ...
    const newCouponID: string = `COUPON${Date.now()}`;
    setCoupons((prev) => ({
      ...prev,
      [newCouponID]: {
...
// Coupon component
const Coupon: React.FC<CouponComponentProps> = ({
  coupon,
  couponName,
  removeCoupon,
  handleCouponChange,
          ...
          {couponTypes[couponName] === 'amountOff' ? (
              ...
              <input
                ...
                onChange={(e) =>
                  handleCouponChange(couponName, {
                    amountOff: parseInt(e.target.value),
                  })
                }
              />
            </div>
            ...
            <div className="flex gap-2">
              ...
              <input
                ...
                onChange={(e) =>
                  handleCouponChange(couponName, {
                    percentOff: parseInt(e.target.value),
                  })
                }
              />
            </div>
